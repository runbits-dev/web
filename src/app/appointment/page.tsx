...
const VERTICAL = 'appointment' as const
const RECOMMENDED_TIER = 'starter' as const

const FEATURES = [
  { key: 'feat1', Icon: CalendarDays },
  { key: 'feat2', Icon: Users },
  { key: 'feat3', Icon: BellRing },
  { key: 'feat4', Icon: RefreshCcw },
]

const STEPS = [
  { number: '01', key: 'step1', Icon: Settings },
  { number: '02', key: 'step2', Icon: Share2 },
  { number: '03', key: 'step3', Icon: CheckCircle2 },
]
...
function Hero() {
  ...
  return (
    <section className="relative pt-32 pb-20 sm:pt-40 sm:pb-28 overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-violet-200/30 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-fuchsia-100/40 rounded-full blur-3xl translate-y-1/2 -translate-x-1/4" />
              ...
              <div className="mt-6 grid grid-cols-7 gap-2">
                {Array.from({ length: 7 }).map((_, i) => (
                  <div key={i} className="text-center">
                    <div className="text-[10px] text-gray-400 font-semibold uppercase mb-1">
                      {['L', 'M', 'X', 'J', 'V', 'S', 'D'][i]}
                    </div>
                      ...
                      {Array.from({ length: 4 }).map((_, j) => (
                        <div
                          ...
                          className={`h-6 rounded ${
                            (i + j) % 3 === 0 ? 'bg-brand-200' : 'bg-gray-100'
                          }`}
                        />
                      ))}
                    </div>
