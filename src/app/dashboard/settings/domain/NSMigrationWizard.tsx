...
import { X, Loader2, Copy, Check, ArrowRight, ExternalLink } from 'lucide-react'
...
type Props = {
  ...
  onClose: () => void
  onConnect: (hostname: string) => Promise<ConnectResult>
  onVerify: (domainId: string) => Promise<{ active: boolean }>
}

export function NSMigrationWizard({
  initialDomain = '',
  onClose,
  onConnect,
  onVerify,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl max-w-lg w-full p-6 relative max-h-[90vh] overflow-y-auto">
        ...
        <h3 className="text-lg font-bold text-slate-900 mb-1">
          {t('settingsDomain.customDomain.wizard.title')}
        </h3>
        ...
        {step === 1 && (
          <div className="mt-4">
            <h4 className="font-semibold text-slate-900 mb-1">
              {t('settingsDomain.customDomain.wizard.step1Title')}
            </h4>
            <p className="text-sm text-slate-600 mb-4">
              {t('settingsDomain.customDomain.wizard.step1Desc')}
            </p>
            <input
              type="text"
              value={hostname}
            ...
            <div className="flex gap-2">
              ...
              >
                {submitting ? (
                  ...
                  <>
                    {t('settingsDomain.customDomain.wizard.step1Cta')}{' '}
        ...
        {step === 2 && result && (
          <div className="mt-4">
            <h4 className="font-semibold text-slate-900 mb-1">
              {t('settingsDomain.customDomain.wizard.step2Title')}
            </h4>
            <p className="text-sm text-slate-600 mb-4">
              {t('settingsDomain.customDomain.wizard.step2Desc')}
            </p>
            ...
            <p className="text-xs text-slate-500 mb-4">
              {t('settingsDomain.customDomain.wizard.step2Hint')}
            </p>
            <button
              onClick={() => setStep(3)}
            ...
            >
              {t('settingsDomain.customDomain.wizard.step2Done')}{' '}
              <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        )}
        ...
        {step === 3 && result && (
          <div className="mt-4">
            <h4 className="font-semibold text-slate-900 mb-1">
              {t('settingsDomain.customDomain.wizard.step3Title')}
            </h4>
            <p className="text-sm text-slate-600 mb-4">
              {t('settingsDomain.customDomain.wizard.step3Desc')}
            </p>
            {verifyMessage && (
            ...
            >
              {submitting ? (
                ...
                t('settingsDomain.customDomain.wizard.step3Cta')
              )}
            </button>
          </div>
        )}
        ...
        {step === 4 && result && (
          <div className="mt-4">
            <h4 className="font-semibold text-slate-900 mb-1 flex items-center gap-2">
              ...
              {t('settingsDomain.customDomain.wizard.step4Title')}
            </h4>
            <p className="text-sm text-slate-600 mb-3">
              {t('settingsDomain.customDomain.wizard.step4Desc')}{' '}
              <a
            ...
            >
              {t('settingsDomain.customDomain.wizard.step4Cta')}
            </button>
          </div>
        )}
      </div>
...
function CopyRow({ value, label }: { value: string; label?: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-2 bg-slate-50 rounded-xl px-3 py-2 ring-1 ring-slate-200">
      <div className="min-w-0 flex-1">
        {label && <p className="text-[10px] text-slate-500 uppercase font-semibold">{label}</p>}
        <p className="font-mono text-xs text-slate-800 truncate">{value}</p>
      </div>
      <button
        onClick={handleCopy}
        className="shrink-0 text-xs font-semibold px-2.5 py-1.5 rounded-lg bg-white text-slate-700 ring-1 ring-slate-200 hover:bg-slate-100 inline-flex items-center gap-1"
      >
        {copied ? (
          <>
            <Check className="w-3 h-3 text-emerald-600" />{' '}
            {t('settingsDomain.copied')}
          </>
        ) : (
          <>
            <Copy className="w-3 h-3" /> {t('settingsDomain.copy')}
          </>
        )}
      </button>
    </div>
