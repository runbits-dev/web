  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        {loading ? <div className="p-8 text-center text-slate-400 text-sm">Cargando...</div> : orders.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead><tr className="bg-slate-50 border-b border-slate-200">
              ...
              <th className="text-right px-6 py-3.5 font-semibold text-slate-600 text-xs uppercase tracking-wider">Subtotal</th>
              <th className="text-right px-6 py-3.5 font-semibold text-slate-600 text-xs uppercase tracking-wider">Fee</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {orders.map(o => (
                <tr key={o.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-right text-slate-600">${(o.subtotal / 100).toFixed(2)}</td>
                  <td className="px-6 py-4 text-right text-slate-600">${(o.platform_fee / 100).toFixed(2)}</td>
